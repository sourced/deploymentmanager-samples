...
def create_subscription(resource_name, spec, topic_name, spec_index):
...
def create_pubsub(resource_name, pubsub_spec):
    ...
    topic_name = pubsub_spec.get('topic')
    topic_id = '{}-topic'.format(resource_name)
    topic = {
        'name': topic_id,
    ...
    subscriptions = [create_subscription(resource_name, spec, topic_id, index)
                     for (index, spec)
